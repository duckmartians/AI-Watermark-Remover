...
在**画笔大小**滑块下方（左侧栏，仅 **Pro** 版）有四个滑块，可在速度与质量之间权衡。将鼠标悬停在任意滑块上可查看提示；你的设置会被记住。

| 滑块 | 控制内容 | 更快 | 更清晰 |
|---|---|---|---|
| **Resolution** | 处理分辨率——最主要的速度杠杆。数值越低 = 越快，被擦除区域会略微模糊。 | ~**50** | **100** |
| **Motion** | 分析帧间运动的彻底程度（对质量影响很小）。 | **2–4** | 20 |
| **Frames** | 一次一起处理多少相邻帧。 | 更低 | 更高 |
| **Reference** | 挑选参考帧时的间隔。 | **更高** | 更低 |

- 想要快？把 **Resolution** 设为 **≈ 50**、**Motion** 设为 **≈ 4**（默认值本就偏向速度）。
- 被擦除区域看起来模糊？把 **Resolution** 调高至 **100**。
- 这些只影响**视频**；对图片没有影响。
...
| 视频非常慢 | 调低 **Resolution** 滑块（≈50）和 **Motion**（≈4）——参见*速度 ⇄ 质量 滑块*一节。强烈建议使用支持 CUDA 的 NVIDIA 显卡（RTX 2060+）——参见*系统要求*。 |
